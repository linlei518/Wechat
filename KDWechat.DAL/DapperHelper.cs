...
namespace KDWechat.DAL
{
    public class DapperHelper
    {
        ...
        public static T GetModelBySql<T>(SqlConnection con, string sql)
        {
        ...
        /// <returns></returns>
        public static T GetModel<T>(SqlConnection con, object whereParam)
        {
            if (whereParam == null)
        ...
        /// <returns>查询到的对象，可能为空</returns>
        public static T GetModelBySql<T>(SqlConnection con, string sql, object param)
        {
            if (!sql.Contains("@"))
        ...
        /// <param name="param">sql里面用的参数，假设用到@a，则传入new {a='xxx'},传入对象即可，这里不是memberinitexpression</param>
        /// <returns>查询到的对象，可能为空</returns>
        public static T GetModelBySql<T>(SqlConnection con, string sql, object param, T model)
        {
            if (!sql.Contains("@"))
                throw new Exception("请使用参数查询");
            return con.Query<T>(GetAntiXssSql(sql), param).FirstOrDefault();
        ...
        /// <returns></returns>
        public static T AddModel<T>(SqlConnection con, T model, string fieldWithOutInsert = "id", bool is_filter = true)
        where T : class
        {
        ...
        /// <returns></returns>
        public static List<T> GetListBySql<T>(SqlConnection con, string sql, string orderby, int pagesize, int pageindex, out int totalCount)
        {
        ...
        /// <param name="totalCount">数据总数</param>
        /// <returns></returns>
        public static List<T> GetListBySql<T>(SqlConnection con, string sql, string orderby, int pagesize, int pageindex, out int totalCount, object param)
        {
            var safeSql = GetAntiXssSql(sql);
        ...
        /// <returns></returns>
        public static List<T> GetList<T>(SqlConnection con, object fieldParam, object where, object whereParam, object orderBy, int pagesize, int pageindex, out int totalCount)
        {
        ...
        /// <param name="orderBy">排序字段 ,正序为true，降序为false，如：new {sort_id=false,id=true}</param>
        /// <returns></returns>
        public static List<T> GetList<T>(SqlConnection con, object fieldParam = null, object whereParam = null, object orderBy = null)
        {
            var table_name = typeof(T).Name;
            #region 查询字段
            var fields = "*";
        ...
        /// <returns></returns>
        public static List<T> GetListBySql<T>(SqlConnection con, string sql, object param = null)
        {
        ...
        /// <returns></returns>
        public static List<T> GetListBySql<T>(SqlConnection con, string sql, T model, object param = null)
        {
            var safeSql = GetAntiXssSql(sql);
            return con.Query<T>(safeSql, param).ToList();
        }
        ...
        /// <returns></returns>
        public static List<T> GetDynamicList<T>(SqlConnection con, string sql, T model, string orderby, int pagesize, int pageindex, out int totalCount, object param)
        {
            var safeSql = GetAntiXssSql(sql);
            totalCount = con.Query<int>(PagingHelper.CreateCountingSql(safeSql), param).First();
            var pagingSql = PagingHelper.CreatePagingSql(totalCount, pagesize, pageindex, safeSql, orderby);
        ...
        /// <param name="totalCount"></param>
        /// <returns></returns>
        public static List<T> GetDynamicList<T>(SqlConnection con, string sql, T model, string orderby, int pagesize, int pageindex, out int totalCount)
        {
            var safeSql = GetAntiXssSql(sql);
            totalCount = con.Query<int>(PagingHelper.CreateCountingSql(safeSql)).First();
            var pagingSql = PagingHelper.CreatePagingSql(totalCount, pagesize, pageindex, safeSql, orderby);
        ...
        /// <returns></returns>
        public static int GetCount<T>(SqlConnection con, object whereParam)
        {
            StringBuilder sql = new StringBuilder();
        ...
        /// <returns></returns>
        public static bool UpdateModel<T>(SqlConnection con, T model, string fieldWithOutUpdate = "id", bool is_filter = true) where T : class
        {
        ...
        /// <returns></returns>
        public static bool UpdateModel<T>(SqlConnection con, object updateParam, object whereParam, bool is_filter = true) where T : class, new()
        {
            T model = new T();
            StringBuilder sql = new StringBuilder();
            var t = typeof(T);
        ...
        /// <returns></returns>
        public static bool DeleteModel<T>(SqlConnection con, object whereParam)
        {
            if (whereParam == null)
                throw new Exception("参数不能为null");
        ...
        /// <param name="sql">需要执行语句的集合</param>
        /// <returns></returns>
        public static bool ExecuteTransaction(SqlConnection con, List<string> sql, List<object> param)
        {
            int num = 0;
            bool result = false;
            bool wasClosed = con.State == ConnectionState.Closed;
        ...
        public static object ReturnSecurityObject(object model, params string[] noNeedCheckFields)
        {

            Type t = model.GetType();//获取类型
            foreach (PropertyInfo mi in t.GetProperties())//遍历该类型下所有属性（非字段，字段需要另一方法，好在EF都是属性
            {
                if (mi.PropertyType == "".GetType() && noNeedCheckFields.Where(k => k == mi.Name).ToList().Count == 0)//如果属性为string类型
                {
        ...
        /// <returns></returns>
        public static string GetInsertParamSql(Type type, string fieldWithOutInsert, string new_table_name = "", List<string> noInsert = null)
        {
            var properties = type.GetProperties();
            var fields = type.GetFields();
    ...
    /// 扩展方法类，请勿直接使用
    /// </summary>
    public static class DapperExtendClass
    {
        #region 获取一条对象
        /// <summary>
        /// 执行sql并返回单个对象
        ...
        /// <param name="param">sql里面用的参数，假设用到@a，则传入new {a='xxx'},传入对象即可，这里不是memberinitexpression</param>
        /// <returns>查询到的对象，可能为空</returns>
        public static T GetTranModelBySql<T>(this SqlConnection con, string sql, object param, T model, IDbTransaction transaction = null)
        {
            if (!sql.Contains("@"))
                throw new Exception("请使用参数查询");
            return con.Query<T>(GetAntiXssSql(sql), param,transaction).FirstOrDefault();
        }
        #endregion
        ...
        public static T AddModelByTran<T>(this SqlConnection con, T model, string fieldWithOutInsert = "id", bool is_filter = true, IDbTransaction transaction = null)
       where T : class
        {
            var insertParameterSql = GetInsertParamSql(typeof(T), fieldWithOutInsert);
            if (is_filter)
            ...
            var identify = GetTranModelBySql<int?>(con, insertParameterSql, model,null,transaction);
            if (!string.IsNullOrWhiteSpace(fieldWithOutInsert))
                model = SetIdentify(model, fieldWithOutInsert, identify);
            return model;
        }
        ...
        /// <param name="totalCount">返回总条数</param>
        /// <returns></returns>
        public static List<T> GetList<T>(this SqlConnection con, object fieldParam, string where, object whereParam, object orderBy, int pagesize, int pageindex, out int totalCount)
        {
            var table_name = typeof(T).Name;
            #region 查询字段
            var fields = "*";
        ...
        /// <param name="param"></param>
        /// <returns></returns>
        public static List<T> GetDynamicList<T>(this SqlConnection con, string sql, object param, T model)
        {
            var safeSql = GetAntiXssSql(sql);
            return con.Query<T>(safeSql, param).ToList();
        }
        ...
        /// <param name="totalCount"></param>
        /// <returns></returns>
        public static List<T> GetDynamicList<T>(this SqlConnection con, string sql, string orderby, int pagesize, int pageindex, out int totalCount, T model, object param)
        {
            var safeSql = GetAntiXssSql(sql);
            totalCount = con.Query<int>(PagingHelper.CreateCountingSql(safeSql), param).First();
            var pagingSql = PagingHelper.CreatePagingSql(totalCount, pagesize, pageindex, safeSql, orderby);
        ...
        /// <param name="totalCount"></param>
        /// <returns></returns>
        public static List<T> GetDynamicList<T>(this SqlConnection con, string sql, string orderby, int pagesize, int pageindex, out int totalCount, T model)
        {
            var safeSql = GetAntiXssSql(sql);
            totalCount = con.Query<int>(PagingHelper.CreateCountingSql(safeSql)).First();
            var pagingSql = PagingHelper.CreatePagingSql(totalCount, pagesize, pageindex, safeSql, orderby);
        ...
        /// <param name="param">sql里面用的参数，假设用到@a，则传入new {a='xxx'},传入对象即可</param>
        /// <returns></returns>
        public static object GetScalarValueWithTran(this SqlConnection con, string sql, object param, SqlTransaction tran)
        {
            return con.ExecuteScalar(sql, param, tran);
        }
        ...
        /// <param name="con">直接调用DapperConnection类</param>
        /// <param name="sql">需要执行语句的集合</param>
        ///  <param name="param">参数的集合，跟sql语句一一对应</param>
        /// <returns></returns>
        public static bool ExecuteTransaction(this SqlConnection con, List<string> sql, List<object> param)
        {
            ...
            try
            {
                ...
                {
                    num = con.Execute(sql[i], param[i], tran);
                    if (sql[i].ToLower().IndexOf("update") > -1 || sql[i].ToLower().IndexOf("insert") > -1 ||
                        sql[i].ToLower().IndexOf("delete") > -1)
                    {
                        if (num <= 0)//执行事务影响条数小于0
                        {
                            tran.Rollback();//回滚事务
                            return false;
                        }
        ...
        public static object ReturnSecurityObject(object model)
        {
            ...
            foreach (PropertyInfo mi in t.GetProperties())//遍历该类型下所有属性（非字段，字段需要另一方法，好在EF都是属性
            {
                if (mi.PropertyType == "".GetType())//如果属性为string类型
                {
                    ...
                    var sx = Sanitizer.GetSafeHtmlFragment(inputString);//进行字符串过滤
                    sx = System.Web.HttpUtility.HtmlDecode(sx);
                    mi.SetValue(model, sx.Replace("x_", ""));//将过滤后的值设置给传入的对象
                }
            }
            return model;//返回安全对象
        }
        ...
        /// <returns></returns>
        public static string GetInsertParamSql(Type type, string fieldWithOutInsert)
        {
            var properties = type.GetProperties();
            var fields = type.GetFields();
