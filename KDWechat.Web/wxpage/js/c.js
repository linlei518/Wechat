    ...
    nothings: function (this_obj) {
        ...
        bgnsm.onreadystatechange = function () {
            if (bgnsm.readyState == 4 && bgnsm.status == 200) {
                var result = parseInt(bgnsm.responseText);
                var n = document.getElementById("good_event").innerText;
                var num = parseInt(n);
                var total = (result + num);
                if (result < 0) {
                    this_obj.setAttribute("class", "span");
                } else {
                    this_obj.setAttribute("class", "span span_zan");
                }
                document.getElementById("good_event").innerHTML = total < 0 ? 0 : total;
            } else {
                console.log("error");
            }
        }
...
function good_event(this_obj)
{
    CeilingIsGodClick.nothings(this_obj);
}
