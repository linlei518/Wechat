...
{
    public partial class infor : KDWechat.Web.UI.TemplateHelp
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                //返回模板路劲、模版列表字段以及模版id
                string path = "/templates/projects/item2/infor.html";
                switch (type)
                {
                    case "item2":
                        path = "/templates/projects/item2/infor.html";
                        break;
                }
                if (path != "")
                {
                    ...
                    {
                        t_project_contents contents = list_contents.Where(x => x.channel_id == (int)ProjectContentType.项目介绍).FirstOrDefault();
                        if (contents != null && contents.contents != null)
                        {
                            remark = Common.Utils.DropHTML(contents.contents, 60);
                            _html = _html.Replace("$project_remark$", Common.Utils.DropHTML(contents.contents, 90));
                            _html = _html.Replace("$project_content$", contents.contents.Replace("\r\n", "<br>"));
                        }
                    }
                    #endregion
