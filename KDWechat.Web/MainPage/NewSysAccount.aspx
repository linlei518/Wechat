<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="NewSysAccount.aspx.cs" Inherits="KDWechat.Web.main.NewAccount" %>
    ...
    <form id="form1" runat="server">
        <section id="main">
            ...
            <div class="listPanel_01 bottomLine">
                ...
                <dl>
                    ...
                    <dd>
                        <asp:DropDownList CssClass="select" ID="ddlAreaType" runat="server">
                            ...
                            <asp:ListItem Value="6">华中</asp:ListItem>
                ...
                <dl id="dd_manage_user" style="display: none" runat="server">
                    <dt>管理用户权限：</dt>
                    <dd>
                        <label class="radioArea">
                            <input runat="server" id="rboManageUserOk" type="radio" class="radio" name="manageUserState" />是</label>
                        <label class="radioArea">
                            <input type="radio" id="rboManageUserFlase" runat="server" class="radio" name="manageUserState" />否</label>
                        <label class="error" id="labManageUser"></label>
                ...
                <dl id="dd_role" style="display: none" runat="server">
                    ...
                    <dd>
                        <div class="jurisdictionSet" id="divRole" style="border: 0;">
                            <ul>
                                <asp:Literal ID="litRole" runat="server"></asp:Literal>
                            </ul>
                            <asp:HiddenField ID="hfactions" runat="server" />
                            <asp:HiddenField ID="hfchild_sys_id" runat="server" />
                            <asp:HiddenField ID="hfchild_sys_name" runat="server" />
                        </div>
                    </dd>
                </dl>
                <dl id="ddl_role_region" style="display: none" runat="server">
                    <dt>选择管理系统：
                    </dt>
                    <dd>
                        <div class="jurisdictionSet" id="divChildSysRole" style="border: 0;">
                            <ul>
                                <asp:Literal ID="lblChildSysRole" runat="server"></asp:Literal>
                            </ul>
                            <asp:HiddenField ID="hfChildSysRole" runat="server" />
                        </div>
                    </dd>
                </dl>
            </div>
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                $("#btnSubmit").click(function () {
                    ...
                    if (!$("#txtAccountName").val()) {
                        ...
                        $("#txtEmail").focus();
                        return false;
                    } else if ($("#ddlUserFlag").val() == "1" && $("#divRole input:checkbox:checked").length <= 0) {
                        showTip.show("请选择可以管理的系统", true);
                        return false;
                    } else if ($("#ddlUserFlag").val() == "2" && $("#divChildSysRole input:checkbox:checked").length <= 0) {
                        showTip.show("请选择可以管理的系统", true);
                        return false;
                    }
                    ...
                    //子系统的权限
                    if ($("#hfchild_sys_name").val() != "") {
                        var list_child = $("#hfchild_sys_name").val().split(',');
                        var temp = "";

                        for (var i = 0; i < list_child.length; i++) {

                            var selector = ".jurisdictionSet .checkbox[name=\"" + list_child[i] + "\"]";
                            $(selector).each(function () {
                                if ($(this).prop("checked") == true) {
                                    var nav_type_id = $(this).prop("id");
                                    var action_list = "";
                    ...
                    var strNav2 = "";
                    if ($("#ddlUserFlag").val() == "2") {
                        $('.jurisdictionSet .checkbox[name="root_child_sys"]').each(function () {
                            if ($(this).prop("checked") == true) {
                                $(this).parent().parent().parent().find('.checkbox').not(this).each(function () {
                                    if ($(this).prop("checked") == true) {
                                        var nav = $(this).val();
            ...
            function change() {
                ...
                if (str == "总部账号") {
                    $("#dd_role").show();
                    $("#dd_manage_user").show();
                    $("#ddl_role_region").hide();
                }
                else if (str == "地区账号") {
                    $("#ddl_role_region").show();
                    $("#dd_role").hide();
                    $("#dd_manage_user").hide();
                }
            }
