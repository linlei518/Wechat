    ...
    public partial class set_qy_admin : Web.UI.BasePage
    {
        ...
        protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName == "sel")
            {
                ...
                var user = DapperConnection.minebea.GetModel<t_wx_fans>(new {open_id = userID});
                if (user != null)
                {
                    Response.Write("<script>parent.SetQyManager('" + user.open_id + "','" + user.nick_name +"')</script>");
                }
                else
                {
        ...
        {
            object where_obj = new { wx_id = wx_id, nick_name = txtKeywords.Value };
            if (!string.IsNullOrEmpty(txtKeywords.Value))
            {
                where_obj = new { wx_id = wx_id, nick_name= txtKeywords.Value };
            }

            var list = DapperConnection.minebea.GetList<t_wx_fans>(null, where_obj);
