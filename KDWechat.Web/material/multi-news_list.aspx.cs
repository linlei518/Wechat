...
{
    public partial class multi_news_list : KDWechat.Web.UI.BasePage
    {
        #region 页面属性
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                CheckUserAuthority((is_public == 1 ? "material_multi_news_public" : "material_multi_news"));
                BindList();
            }
        }

        protected string GetChildList(object parent_id)
        {
            string childList = KDWechat.BLL.Chats.wx_news_materials.GetChildList(2, Convert.ToInt16(parent_id));
        ...
        private void BindList()
        {
            ...
            if (is_public == 1)
            {
                Query.Append(string.Format("select id,title,cover_img,group_id,status,create_time,content_html,img_list from t_wx_news_materials where is_public=1 and channel_id={0} and par_id=0", channel_id));
            }
            else
            {
                Query.Append(string.Format("select id,title,cover_img,group_id,status,create_time,content_html,img_list from t_wx_news_materials where wx_id={0} and channel_id={1} and par_id=0", wx_id, channel_id));
            }
            ...
            repList.DataBind();
            string pageUrl = string.Format("multi-news_list.aspx?key={0}&group_id={1}&status={2}&is_pub={3}&page=__id__&m_id={4}", HttpUtility.UrlEncode(key), group_id, status, is_pub, m_id);
        ...
        protected void repList_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            if (e.CommandName == "status")
            {
                ...
                AddLog("更改多图文素材状态：" + lblTitle.Text + " 为" + (status == 1 ? "启用" : "禁用"), LogType.修改);
                Response.Redirect("multi-news_list.aspx?key=" + HttpUtility.UrlEncode(key) + "&status=" + this.status + "&group_id=" + group_id + "&page=" + page + "&is_pub=" + is_pub + "&m_id=" + m_id);
            }
            else if (e.CommandName == "del")
            {
                ...
                {
                    if (KDWechat.BLL.Chats.wx_news_materials.Delete(id, true))
                    {
                        try
                        {
                            string img = (e.Item.FindControl("hf_img") as HiddenField).Value.TrimEnd(',');
                            string[] list_img = img.Split(new char[]{','});
                    ...
                    JsHelper.AlertAndRedirect("删除成功", "multi-news_list.aspx?key=" + HttpUtility.UrlEncode(key) + "&status=" + this.status + "&group_id=" + group_id + "&page=" + page + "&is_pub=" + is_pub + "&m_id=" + m_id);
                }
            }
        ...
        {
            List<DAL.t_wx_news_materials> list = BLL.Chats.wx_news_materials.GetList();
            if (list!=null)
            {
                foreach (t_wx_news_materials m in list)
                {
                    CreateNewsQrCode(m.id);
                }
            }
            JsHelper.AlertAndRedirect("生成成功", "multi-news_list.aspx?key=" + HttpUtility.UrlEncode(key) + "&status=" + this.status + "&group_id=" + group_id + "&page=" + page + "&is_pub=" + is_pub + "&m_id=" + m_id);
        }
    }
}