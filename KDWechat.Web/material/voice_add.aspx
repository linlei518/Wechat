<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="voice_add.aspx.cs" Inherits="KDWechat.Web.material.voice_add" %>
    ...
    <form id="form1" runat="server">
        ...
        <section  <%=isnotop==true?"id=\"bombboxMain\" style='top:5px;'":"id=\"main\"" %>>
	        ...
	        <div class="listPanel_01 bottomLine">
		        <dl>
			        <dt>语音标题：</dt>
			        <dd><asp:TextBox ID="txtTitle" runat="server" class="txt " MaxLength="50"></asp:TextBox><i>*</i><em>不超过50个字</em></dd>
		        </dl>
		        ...
		        <dl>
                 ...
                 <dl>
			        ...
			        <dd>
                        ...
                        <asp:TextBox ID="txtFile" runat="server"     class="txt " ReadOnly="true"></asp:TextBox>
                        <%--调用编辑器的单上传模式 --%>
                        <%--<input type="button" id="btnUpload"    value="浏览...">--%>
                        <%--调用编辑器的上传+选择模式--%>
                       
                         
                  
                        <input type="button"  id="btnUpload" class="btn btn6" value="浏览...">  <label id="lblImg" class="error" style="display: inline;"></label>

                        <%--自己上传调用方法，Upload_File方法在function.js里需引用，目前file控件样式有问题，这里暂不用--%>
				       <%-- <input type="file" class="file" id="file_img" name="file_img" onchange="Upload_File('file_img','txtFile','hf_type','hf_size','hf_name','<%=wx_og_id %>',1,1,2)">--%>
				        <br /><i>*</i><em>大小：不超过2M, 长度: 不超过60s, 格式: MP3, <%--wma, wav,--%> AMR</em>
			        </dd>
		        </dl>
        ...
        <script type="text/javascript">
            KindEditor.ready(function (K) {
                ...
                <%--调用编辑器的上传加载开始,folder参数表示当前微信号的文件夹--%>
                  var editor = K.editor({
                      uploadJson: '../handles/upload_ajax.ashx?action=EditorFile&IsWater=1&folder=<%=folder%>&upload_type=<%=(int)KDWechat.Common.UploadType.语音%>',
                      fileManagerJson: '../handles/upload_ajax.ashx?action=ManagerFile&folder=<%=folder%>&upload_type=<%=(int)KDWechat.Common.UploadType.语音%>',
                      ...
                      editor.loadPlugin('insertfile', function () {
                          editor.plugin.fileDialog({
                              ...
                              clickFn: function (url, title, width, height, border, align) {
                                  K('#txtFile').val(url);
                                  K('#hf_name').val(url);
                                  //K('.sound').html("<a class=\"audio\" href=\"#\" onClick=\"audioControl(this,'"+url+"')\" title=\"点击试听\"></a>");
                                  editor.hideDialog();
        ...
        <script type="text/javascript">
            function ckName(value) {
                $.ajax({
                    type: "POST",
                    async: false,
                    url: "/handles/wechat_ajax.ashx?action=check_exists_name&channel_id=2",
            ...
            $(function () {
                $("#txtTitle").blur(function () {
                    if (!$(this).val()) {
                        showTip.show("请输入语音标题", true);

                    } else {
                        if (!ckName($(this).val())) {
                            showTip.show("语音标题已存在", true);
                        }
