<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="retrans_keyword_list.aspx.cs" Inherits="KDWechat.Web.keyworld.retrans_keyword_list" %>
        ...
        <section id="main">
            ...
            <div class="titlePanel_01">
                <div class="btns" runat="server" id="div_add">
                    <a href="javascript:bombbox.openBox('retrans_keyword_detail.aspx');" class="btn btn3"><i class="add"></i>新建第三方关键词</a>
                </div>
            </div>
            ...
            <div class="searchPanel_01">
                ...
                <div class="filterField">
                    ...
                    <asp:DropDownList ID="ddlServer" CssClass="select" AutoPostBack="true" OnSelectedIndexChanged="ddlServer_SelectedIndexChanged" AppendDataBoundItems="true" runat="server">
            ...
            <div class="tablePanel_01">
                <table cellpadding="0" cellspacing="0" class="table">
                    <thead>
                        <tr>
                            <th class="name">名称</th>
                            <th class="info info1" style="width:60px;" >第三方服务</th>
                    ...
                    <tbody>
                        <asp:Repeater ID="Repeater1" runat="server">
                            <ItemTemplate>
                                <tr>
                                    <td class="name">
                                        <a href='retrans_server_detail.aspx?id=<%#Eval("id") %>&m_id=<%=m_id %>'><%#Eval("keyword") %></a>
                                    </td>
                                    <td class="info info1" ><%#Eval("title") %></td>
                                    <td class="info info1" ><%#Eval("retrans_times") %></td>
                                    <td class="time"><%#Eval("create_time") %></td>
                                    <td class="control">
                                        <a href="javascript:bombbox.openBox('retrans_keyword_detail.aspx?id=<%#Eval("id") %>');" class="btn btn6">编辑</a>
                                    </td>
                                </tr>
                            </ItemTemplate>
                            <FooterTemplate>
            ...
            function selectStartDate() {
                ...
                WdatePicker(
                {
                    ...
                    onclearing: function () {
                        ...
                        txtend_date.value = "";
                        txtbegin_date.value = "";
                        parent.location.replace("retrans_keyword_list.aspx?key=<%=key%>&beginDate=&endDate=&m_id=<%=m_id%>&sv=<%=sv%>");
                    },
                    doubleCalendar: true,
                    isShowClear: true,
                    readOnly: true,
                ...
                WdatePicker({
                    ...
                    onpicked: function (dp) {
                        if (txt_date_show.value.length > 0) {
                            txt_date_show.value += " — " + dp.cal.getNewDateStr();
                            location.replace("retrans_keyword_list.aspx?key=<%=key%>&beginDate=" + $("#txtbegin_date").val() + "&endDate=" + $("#txtend_date").val() + "&m_id=<%=m_id%>&sv=<%=sv%>");
                        } else {
                            txt_date_show.value = dp.cal.getNewDateStr();
                        }
